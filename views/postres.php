...
$sql = $conexion->query("SELECT * FROM postres");
...
.page-title-dot {
    ...
    border-radius: 50%; background: #f9a8d4;
    flex-shrink: 0;
}

.count-badge {
    font-size: 12px; padding: 5px 12px;
    border-radius: 50px; color: #f9a8d4;
...
.form-card input:focus { border-color: #f9a8d4; outline: none; }
...
.img-upload-label:hover { border-color: #f9a8d4; color: #f9a8d4; }
.img-upload-label.has-file { border-color: #f9a8d4; color: #f9a8d4; }
#img-input { display: none; }

.btn-agregar {
    height: 42px; padding: 0 20px; border: none;
    border-radius: 10px; background: #f9a8d4;
...
.btn-agregar svg { width: 15px; height: 15px; stroke: #4a0e2e; }
...
.postres-table tbody td {
    padding: 12px 16px;
    border-top: 1px solid rgba(255,255,255,.04);
    color: #e2e8f0; vertical-align: middle;
}

.postres-table tbody tr:hover td { background: #1b1e28; }
...
.postre-thumb-placeholder svg { width: 20px; height: 20px; stroke: #334155; }

.precio-display {
    display: inline-flex; align-items: center;
    gap: 4px; color: #f9a8d4;
...
.precio-input:focus, .edit-input:focus { border-color: #f9a8d4; }
...
.btn-change-img:hover { border-color: #f9a8d4; color: #f9a8d4; }
...
.btn-edit:hover { background: rgba(236,72,153,.22); color: #f9a8d4; }
.btn-edit.saving {
    background: rgba(249,168,212,.12); color: #f9a8d4;
...
.toast-msg {
    ...
    color: #f9a8d4; padding: 10px 18px;
    border-radius: 12px;
    font-size: 13px; font-weight: 500;
...
<div class="content-area">
    ...
    <!-- FORM AGREGAR -->
    <form action="../controllers/postresController.php"
          method="POST"
        ...
        <div class="form-grid">
            ...
            <div class="input-wrap">
                ...
                <input type="text" name="nombre"
                       placeholder="Nombre del postre" required>
            </div>
    ...
    <div class="table-card">
        <div class="table-responsive">
            <table class="postres-table" id="postres-table">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Imagen</th>
                ...
                <tr class="empty-row">
                    <td colspan="5">
                        <div class="empty-icon">
                            ...
                            No hay postres registrados aún
                        </div>
                    </td>
                </tr>
                <?php endif; ?>
                ...
                <tr>
                    ...
                    <td class="td-img">
                        <div class="img-edit-wrap">
                            ...
                            <?php if (!empty($row['imagen'])): ?>
                                <img src="../uploads/postres/<?= htmlspecialchars($row['imagen']) ?>"
                                     class="postre-thumb"
                                     id="thumb-<?= $row['id'] ?>"
                                     onerror="this.onerror=null;this.src='../assets/img/<?= htmlspecialchars($row['imagen']) ?>'"
                                     alt="<?= htmlspecialchars($row['nombre']) ?>">
                            <?php else: ?>
                                <div class="postre-thumb-placeholder" id="thumb-<?= $row['id'] ?>">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                                        <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/>
                                        <circle cx="8.5" cy="8.5" r="1.5"/>
                                        <polyline points="21 15 16 10 5 21"/>
                    ...
                    <td class="td-actions">
                        <div class="btn-actions">
                            ...
                            <a href="../controllers/postresController.php?eliminar=<?= $row['id'] ?>"
                               class="btn-delete"
                               onclick="return confirm('¿Eliminar este postre?')">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
...
<div class="toast-msg" id="toast">
    ...
    <span id="toast-text">Postre actualizado</span>
</div>

<script>
document.getElementById("count-badge").querySelector("span").textContent =
    document.querySelectorAll(".postres-table tbody tr:not(.empty-row)").length + " registros";
...
function showToast(msg, isError) {
    ...
    document.getElementById("toast-text").textContent = msg || "Postre actualizado";
    if (isError) {
        t.style.borderColor = "rgba(239,68,68,.25)";
        t.style.color = "#f87171";
        t.querySelector("svg").style.stroke = "#f87171";
    } else {
        t.style.borderColor = "rgba(249,168,212,.25)";
        t.style.color = "#f9a8d4";
        t.querySelector("svg").style.stroke = "#f9a8d4";
    }
...
document.querySelectorAll(".guardar").forEach(function(btn) {
    ...
    btn.addEventListener("click", function() {
        ...
        fetch("../controllers/postresController.php", { method: "POST", body: fd })
            ...
            if (data.trim() === "ok") {
                ...
                if (imgFile) {
                    ...
                    if (thumbWrap.tagName === "IMG") {
                        ...
                        img.src = url; img.className = "postre-thumb"; img.id = "thumb-" + id;
                        thumbWrap.replaceWith(img);
                    }
                }

                showToast("Postre actualizado correctamente");
